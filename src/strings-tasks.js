...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, indexSubStr) + str.slice(indexSubStr + value.length);
}
...
function formatTime(minutes, seconds) {
  return `${minutes.toString().padStart(2, '0')}:${seconds
    .toString()
    .padStart(2, '0')}`;
  // return minutes.padStart(3, '"0') + ':' + seconds.padStart(3, '0') + '"';
  // return seconds < 10
  //   ? '0'.concat(minutes, ':0', seconds)
  //   : '0'.concat(minutes, ':', seconds);
...
function isPalindrome(str) {
  return (
    str
      .split('')
      .filter((item) => item !== ' ' && item.charCodeAt() > 64)
  ...
  return str
    ...
    .map((item) => {
      if (item.charCodeAt() > 64 && item.charCodeAt() < 123) {
        if (
          (item.charCodeAt() > 77 && item.charCodeAt() < 97) ||
          item.charCodeAt() > 109
        ) {
